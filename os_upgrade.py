...
def upgrade_os_and_license(equipment_manufacturer, os):
    if equipment_manufacturer == 'pan':
        ...
        if 'no' not in version or 'n' != version:
            file.write('request global-protect-client software check\n')
            file.write('request global-protect-client software download version ' + version)
            file.write('sleep 2\n')
            file.write('request global-protect-client software activate version ' + version + '\n')
            file.write('y\n')
